...
NSArray *PerformHTMLXPathQueryWithEncoding(NSData *document, NSString *query,NSString *encoding)
{
    ...
    doc = htmlReadMemory((const char *)[document bytes], (int)[document length], "", encoded, HTML_PARSE_NOWARNING | HTML_PARSE_NOERROR);
    if (doc == NULL) {
...
NSArray *PerformXMLXPathQueryWithEncoding(NSData *document, NSString *query,NSString *encoding)
{
    ...
    doc = xmlReadMemory((const char*)[document bytes], (int)[document length], "", encoded, XML_PARSE_RECOVER);
